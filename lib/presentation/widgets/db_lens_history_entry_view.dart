...
/// Detail satu entry riwayat: diff per-field (before/after berdampingan)
/// atau raw JSON, dengan toggle di header.
///
/// Widget biasa — bisa ditempel di `Scaffold` biasa maupun dibungkus
/// `showModalBottomSheet` lewat [DbLensHistoryEntryView.showAsSheet].
...
/// Menampilkan diff per-field antara "before" dan "after" satu [HistoryEntry]
/// secara berdampingan (kiri = before, kanan = after) dengan highlight warna.
class DbLensHistoryDiffView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        for (final field in fields) _buildFieldRow(field),
      ],
    );
  }
  ...
  Widget _buildFieldRow(DbLensDiffField field) {
